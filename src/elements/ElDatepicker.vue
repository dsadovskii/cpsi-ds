...
export default {
  ...
  computed: {
    ...
    date: {
      get() {
        return momentTz.tz(moment(this.value, 'YYYY-MM-DD HH-mm-SS')._i, this.timezone).format(this.format)
      },
      set(value) {
        this.$emit('input', value ? momentTz.tz(moment(value, this.format), 'UTC').format() : value)
      },
    },
  },
  methods: {
